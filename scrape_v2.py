...
class Fragrance():
    ...
    def calc_prices(self):
        price = Scrapes.scrape_fs_price(self.properties.get("fs_url"), fs_discount)
        self.properties["prices"]["fs_price"] = price
        price = Scrapes.scrape_fd_price(self.properties.get("fd_url"), fd_discount)
        self.properties["prices"]["fd_price"] = price
        price = Scrapes.scrape_jl_price(self.properties.get("jl_url"), jl_discount)
    ...
    def find_diff(self):
        if len(self.properties["present_prices"]) == 0:
            self.compare()
        sorted_prices = sorted(self.properties["present_prices"].items(), key = lambda x : x[1])
        if len(sorted_prices) > 1:
            self.properties["difference"] = abs(sorted_prices[0][1] - sorted_prices[1][1])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--jl", type=int,
                        help="fragrance direct discount", default=0)
